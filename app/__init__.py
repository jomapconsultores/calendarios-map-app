...
def dashboard():
    ...
    def login():
        if request.method == 'POST':
            ...
            if users and check_password_hash(users[0]['password_hash'], pw):
                login_user(User(users[0])); return redirect('/dashboard')
    ...
    def register():
        if request.method == 'POST':
            ...
            name = request.form.get('full_name'); cals = request.form.getlist('calendars')
            ...
            if result:
                for cal_id in cals:
                    app.supabase.insert('calendar_permissions', {'user_id': result[0]['id'], 'calendar_id': cal_id, 'status': 'pending'})
    ...
    def admin_users():
        ...
        users = app.supabase.get('users'); all_cals = app.supabase.get('calendar_config')
        for u in users:
        ...
        for p in pending_raw:
            if p['user_id'] not in seen:
                ...
                if uu: item['user_name'] = uu[0].get('full_name', ''); item['user_email'] = uu[0].get('email', '')
                all_up = app.supabase.get('calendar_permissions', {'user_id': p['user_id'], 'status': 'pending'})
                cal_ids = [ap['calendar_id'] for ap in all_up]
                item['calendars'] = [c for c in all_cals if c['calendar_id'] in cal_ids]
                pending.append(item)
        return render_template('admin_users.html', users=users, calendarios=all_cals, pending=pending)
    ...
    def admin_update_user(uid):
        ...
        flash('✅ Usuario actualizado', 'success'); return redirect('/admin/users')

    @app.route('/admin/user/delete/<uid>', methods=['POST'])
    @login_required
    def admin_delete_user(uid):
        if not is_admin(): return {'success': False}
        for p in app.supabase.get('calendar_permissions', {'user_id': uid}): app.supabase.delete('calendar_permissions', p['id'])
        app.supabase.delete('users', uid)
        flash('🗑️ Usuario eliminado', 'success'); return redirect('/admin/users')

    @app.route('/admin/approve-one/<pid>', methods=['POST'])
    @login_required
    def admin_approve_one(pid):
        if not is_admin(): return {'success': False}
        app.supabase.update('calendar_permissions', pid, {'status': 'approved'}); return {'success': True}

    @app.route('/admin/reject-one/<pid>', methods=['POST'])
    @login_required
    def admin_reject_one(pid):
        if not is_admin(): return {'success': False}
        app.supabase.update('calendar_permissions', pid, {'status': 'rejected'}); return {'success': True}
    ...
    @login_required
    def api_events():
        if is_admin(): events = app.supabase.get('appointments')
        else:
            ucal = [c['calendar_id'] for c in get_user_calendars(app, current_user.id)]; events = []
    ...
    def api_pending():
        ...
        if is_admin(): pending = [a for a in events if a.get('status') == 'pending']
        else:
